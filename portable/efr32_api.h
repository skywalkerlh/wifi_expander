...
typedef struct {
    state_t cur_state;
    mble_gap_connect_t target_to_connect;
} target_connect_t;

typedef struct {
    bool                is_running;
    uint8_t             timer_id;
    mble_timer_mode    mode;
    mble_timer_handler handler;
    void                *args;
} timer_item_t;

typedef struct {
    mble_handler_t handler;
    void *args;
} handler_item_t;

void mble_stack_event_handler(struct gecko_cmd_packet* event);
